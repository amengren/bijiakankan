...
if __name__ == '__main__':
    # print get_accounts()
    strTime = "2018-11-9 09:00:00"
    t1 = strTime2ts(strTime)
    # t2 = t1 - 24*60*60
    kline = get_kline("ethusdt", "60min", 150)
    #间隔小时数
    t2 = int(math.floor((kline["ts"]/1000 - t1) / 3600))
    print t2
    #获取两个时间段数据
    data = kline["data"]
    d1 = data[t2]
    ...
    print d2
    #计算出涨幅 （今天-昨天）/昨天 * 100
    zf = (d1["close"]-d2["close"]) / d2["close"] * 100
    print strTime+" 涨跌幅："+ str(round(zf,2)) + "%"
    # print get_symbols()
